	...
	for (i = 0; i < win->buffer.width; ++i)
	  {
	    ...
	    if (i < win->buffer.width / 2)
	      v |= (int)((1 - (float)i / (win->buffer.width / 2)) * 15) << 0;
	    else
	      v |= (int)(((float)(i - win->buffer.width / 2) / (win->buffer.width / 2)) * 15) << 4;

	    if (j < win->buffer.height / 2)
	      v |= (int)(((float)j / (win->buffer.height / 2)) * 15) << 12;
	    else
	      {
		v |= 15 << 12;
		v |= (int)(((float)(j - win->buffer.height / 2) / (win->buffer.height / 2.0)) * 15) << 8;
	      }
