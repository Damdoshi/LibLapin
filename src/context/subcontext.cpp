...
t_bunny_response	bunny_subcontext_message(const t_bunny_network_info *inf,
						 const void		*buffer,
						 size_t			siz,
						 void			*data)
{
  ...
  if (x->subcontext.netmessage && x->subcontext.netcom)
    return (x->subcontext.netmessage(inf, buffer, siz, x->main_structure));
  return (GO_ON);
}

t_bunny_response	bunny_subcontext_connect(const t_bunny_network_info *inf,
						 ...
						 void			*data)
{
  ...
  if (x->subcontext.netconnect && x->subcontext.netcom)
    return (x->subcontext.netconnect(inf, state, x->main_structure));
  return (GO_ON);
}
