				      ...
				      void		*data)
{
  ...
  scream_log_if(PATTERN "Entering)", window, nwin, freq, data);
  ...
  while (rep == GO_ON)
      ...
      while (now - prev > delay)
	{
		...
		while (win->window->pollEvent(event))
		  {
			...
			if (gl_callback.close)
			  {
			    scream_log_if(PATTERN "close window %p)", window, nwin, freq, data, gl_window);
		    ...
		    if (event.type == sf::Event::GainedFocus && gl_callback.get_focus)
		      {
			scream_log_if(PATTERN "gain_focus %p)", window, nwin, freq, data, gl_window);
		    ...
		    if (event.type == sf::Event::LostFocus && gl_callback.lost_focus)
		      {
			scream_log_if(PATTERN "lost_focus %p)", window, nwin, freq, data, gl_window);
			...
			scream_log_if(PATTERN "reisze %p)", window, nwin, freq, data, gl_window);
			...
			if (event.type == sf::Event::KeyPressed)
			  {
			    ...
			    scream_log_if(PATTERN "keydown)", window, nwin, freq, data);
			...
			else if (event.type == sf::Event::KeyReleased)
			  {
			    ...
			    scream_log_if(PATTERN "keyup)", window, nwin, freq, data);
			    if ((rep = gl_callback.key
		    ...
		    if (gl_callback.type != NULL && event.type == sf::Event::TextEntered)
		      {
			scream_log_if(PATTERN "text)", window, nwin, freq, data);
			...
			if ((connect = (event.type == sf::Event::JoystickConnected)) ||
			    event.type == sf::Event::JoystickDisconnected)
			  {
				...
				scream_log_if(PATTERN "joyconnect)", window, nwin, freq, data);
			      }
			    else
			      {
				gl_joystick[joyid].connected = false;
				free((char*)gl_joystick[joyid].name);
				scream_log_if(PATTERN "joydisconnect)", window, nwin, freq, data);
			      }
			...
			if (gl_callback.button != NULL)
			  {
				...
				scream_log_if(PATTERN "joydown)", window, nwin, freq, data);
				...
				scream_log_if(PATTERN "joyup)", window, nwin, freq, data);
				if ((rep = gl_callback.button
				     (GO_UP,
				      event.joystickButton.joystickId,
				      event.joystickButton.button,
			...
			if (gl_callback.axis != NULL)
			  {
				...
				scream_log_if(PATTERN "joymove)", window, nwin, freq, data);
			...
			if (event.type == sf::Event::MouseButtonPressed)
			  {
			    ...
			    scream_log_if(PATTERN "mousedown)", window, nwin, freq, data);
			...
			else if (event.type == sf::Event::MouseButtonReleased)
			  {
			    ...
			    scream_log_if(PATTERN "mouseup)", window, nwin, freq, data);
			    if ((rep = gl_callback.click
			...
			if (gl_callback.wheel != NULL)
			  {
			    scream_log_if(PATTERN "mousewheel)", window, nwin, freq, data);
			...
			if (gl_callback.move != NULL)
			  {
			    scream_log_if(PATTERN "mousemove)", window, nwin, freq, data);
	  ...
	  if (gl_callback.loop != NULL)
	    {
	      scream_log_if(PATTERN "loop)", window, nwin, freq, data);
      ...
      if (once && gl_callback.display != NULL)
	{
	  if (delay - (now - prev) > 0.05 * delay || display_cnt > 20)
	    {
	      display_cnt = 0;
	      scream_log_if(PATTERN "display)", window, nwin, freq, data);
